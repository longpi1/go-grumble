...
var addCmd = &grumble.Command{
	...
	//相关flag定义
	Flags: func(f *grumble.Flags) {
		f.Int("i", "id", configs.DefaultNum, "员工id")
		f.String("n", "name", "", "名称")
...
var listCmd = &grumble.Command{
	...
	Help: "查看员工列表",
	Flags: func(f *grumble.Flags) {
		f.Int("k", "key", configs.DefaultNum, "查找类型：请输入(1-6)  1 全部员工信息, 2 按姓名过滤员工信息,  3 按部门过滤员工信息,  4 按职位过滤员工信息,   5 根据员工Id排序,  6 根据员工入职日期排序")
...
func judgeId(c *grumble.Context) bool {
	if c.Flags.Int("id") == configs.DefaultNum {
		return false
	}
	return true
}
